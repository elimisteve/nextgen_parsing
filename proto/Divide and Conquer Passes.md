...
(only the first grammar needs to be human readable, the intermediate grammars need only be parsable by a machine)
...
# Legend
   ...
   `⟪+ name="ch",kind=channel,scope=global,value=5⟫` - data descriptor (def datum)
   `⟪name="ch",kind=channel,scope=global,indirection=1,value=5⟫` - data descriptor usage (datum usage)
...
Human readable source code is input into the compiler pipline ...
